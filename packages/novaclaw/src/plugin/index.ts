    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const client = createNovaclawClient({
          ...
          // hey-api types fetch as the GLOBAL fetch (with Bun preconnect); the in-process adapter uses webHandler which checks prebuilt at call time.
          ...(serverUrl ? {} : { fetch: (async (request: Request) => HttpApiApp.webHandler()(request, HttpApiApp.context)) as unknown as typeof fetch }),
        })
